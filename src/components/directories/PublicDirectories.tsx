...
import { CollectionTags } from "../manifest/CollectionTags";
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {collections.map((collection) => {
            ...
            return <Link key={collection.collectionId} to={publicCollectionPath(publisherSlug, collectionSlug)} className="group grid gap-4 px-6 py-5 transition hover:bg-blue-50/60 dark:hover:bg-blue-950/20 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
              <div><div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-blue-600"><Stack size={15} /> {collection.presentation?.publisherName || publisherSlug}</div><h3 className="mt-1 text-base font-black text-slate-950 dark:text-white">{collection.presentation?.title || collection.batchLabel}</h3><p className="mt-2 text-xs leading-5 text-slate-500">{collection.presentation?.description || collection.description}</p><CollectionTags tags={collection.presentation?.tags} /></div>
